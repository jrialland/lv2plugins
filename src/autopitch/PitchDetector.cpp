...
float Yin_getPitch(Yin *yin, float *buffer) {
  ...
  if (tauEstimate != -1) {
    pitchInHertz =
        yin->samplingRate / Yin_parabolicInterpolation(yin, tauEstimate);
  }
...
PitchDetector::PitchDetector(uint32_t sample_rate_)
    : sample_rate(sample_rate_), buffer(nullptr), buffer_size(0) {}
