...
float Yin_getPitch(Yin *yin, float* buffer){
	...
	/* Step 5: Interpolate the shift value (tau) to improve the pitch estimate. */
	if(tauEstimate != -1){
		pitchInHertz = yin->samplingRate / Yin_parabolicInterpolation(yin, tauEstimate);
	}
...
PitchDetector::PitchDetector(uint32_t sample_rate_) : sample_rate(sample_rate_), buffer(nullptr), buffer_size(0) {
} 
...
float* PitchDetector::get_buffer(uint32_t needed) {
...
float PitchDetector::detect_pitch(uint32_t nsamples, float* input) {
    Yin yin;
    yin.samplingRate = sample_rate;
	yin.bufferSize = nsamples;
	yin.halfBufferSize = nsamples / 2;
    yin.yinBuffer = get_buffer(yin.halfBufferSize);
	yin.probability = 0.0;
	yin.threshold = 0.05;
    memset(yin.yinBuffer, 0, yin.halfBufferSize * sizeof(float));
    return Yin_getPitch(&yin, input);
}